...
async def start_withdrawal_form(callback_query: types.CallbackQuery, state: FSMContext):
    payment_method = callback_query.data.upper()
    await state.update_data(payment_method=payment_method, user_id=callback_query.from_user.id)  # Store user ID
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await message.answer("3️⃣ Enter the **wallet number** you will withdraw to:")
...
async def process_wallet_number(message: types.Message, state: FSMContext):
    await state.update_data(wallet_number=message.text)
    await message.answer("4️⃣ Enter the **Amount** you want to withdraw:")
...
async def process_amount(message: types.Message, state: FSMContext):
    await state.update_data(amount=message.text)
    await message.answer("5️⃣ Enter the **Withdrawal Code** (you can get this from your withdrawal request details):")

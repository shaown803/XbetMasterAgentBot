...
# Withdrawal states
class WithdrawForm(StatesGroup): payment_method = State() player_id = State() name = State() wallet_number = State() amount = State() withdraw_code = State() confirmation = State()
...
await bot.send_message(callback_query.from_user.id, "1️⃣ Send your **Player ID**:")
await WithdrawalForm.player_id.set()
...
keyboard = InlineKeyboardMarkup().add(
    InlineKeyboardButton("✅ SUBMIT", callback_data="submit_withdraw"),
    InlineKeyboardButton("❌ CANCEL", callback_data="cancel")
)
await message.answer(confirm_text, reply_markup=keyboard)
...
admin_keyboard = InlineKeyboardMarkup().add(
    InlineKeyboardButton("✅ APPROVE", callback_data=f"approve_withdraw_{user_data['user_id']}"),
    InlineKeyboardButton("❌ REJECT", callback_data=f"reject_withdraw_{user_data['user_id']}")
)
await bot.send_message(config.ADMIN_GROUP_ID, admin_text, reply_markup=admin_keyboard)
await state.finish()

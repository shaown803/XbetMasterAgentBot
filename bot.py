...
class WithdrawForm(StatesGroup): payment_method = State() player_id = State() name = State() wallet_number = State() amount = State() withdrawal_code = State() confirmation = State()
 
# ✅ Handle Withdraw Button Click
 
@dp.message_handler(lambda message: message.text == "💳 Withdraw") async def show_withdrawal_address(message: types.Message): text = "\U0001F4B0 **Withdrawal Address**\n\n"  "🏙 **City:** Chittagong\n"  "🛣 **Street:** Dorbesh Baba" keyboard = InlineKeyboardMarkup().add(InlineKeyboardButton("➡ Next", callback_data="withdraw_next")) await message.answer(text, reply_markup=keyboard)
 
# ✅ Select Payment Method
 
@dp.callback_query_handler(lambda query: query.data == "withdraw_next") async def select_withdrawal_method(callback_query: types.CallbackQuery): keyboard = InlineKeyboardMarkup(row_width=1) keyboard.add( InlineKeyboardButton("📌 BKASH", callback_data="withdraw_bkash"), InlineKeyboardButton("📌 NAGAD", callback_data="withdraw_nagad"), InlineKeyboardButton("📌 ROCKET", callback_data="withdraw_rocket"), InlineKeyboardButton("📌 UPAY", callback_data="withdraw_upay") ) await callback_query.message.edit_text("Select a withdrawal method:", reply_markup=keyboard)
 
# ✅ Start Withdrawal Form
 
@dp.callback_query_handler(lambda query: query.data.startswith("withdraw_")) async def start_withdraw_form(callback_query: types.CallbackQuery, state: FSMContext): payment_method = callback_query.data.split("_")[1] await state.update_data(payment_method=payment_method, user_id=callback_query.from_user.id) await bot.send_message(callback_query.from_user.id, "1️⃣ Send your **Player ID**:") await WithdrawForm.player_id.set()
 
# ✅ Step 1: Collect Player ID
 
@dp.message_handler(state=WithdrawForm.player_id) async def process_withdraw_player_id(message: types.Message, state: FSMContext): await state.update_data(player_id=message.text) await message.answer("2️⃣ Send your **Name**:") await WithdrawForm.name.set()
 
# ✅ Step 2: Collect Name
 
@dp.message_handler(state=WithdrawForm.name) async def process_withdraw_name(message: types.Message, state: FSMContext): await state.update_data(name=message.text) await message.answer("3️⃣ Enter your **Wallet Number**:") await WithdrawForm.wallet_number.set()
 
# ✅ Step 3: Collect Wallet Number
 
@dp.message_handler(state=WithdrawForm.wallet_number) async def process_withdraw_wallet(message: types.Message, state: FSMContext): await state.update_data(wallet_number=message.text) await message.answer("4️⃣ Enter the **Amount**:") await WithdrawForm.amount.set()
 
# ✅ Step 4: Collect Amount
 
@dp.message_handler(state=WithdrawForm.amount) async def process_withdraw_amount(message: types.Message, state: FSMContext): await state.update_data(amount=message.text) await message.answer("5️⃣ Enter the **Withdrawal Code**:") await WithdrawForm.withdrawal_code.set()
 
# ✅ Step 5: Confirm Submission
 
@dp.message_handler(state=WithdrawForm.withdrawal_code) async def process_withdraw_code(message: types.Message, state: FSMContext): await state.update_data(withdrawal_code=message.text) user_data = await state.get_data()
 `confirm_text = (f"📌 **Withdrawal Submission**\n\n"                 f"👤 **Player ID:** {user_data['player_id']}\n"                 f"📛 **Name:** {user_data['name']}\n"                 f"🏦 **Payment Method:** {user_data['payment_method'].upper()}\n"                 f"📞 **Wallet Number:** {user_data['wallet_number']}\n"                 f"💰 **Amount:** {user_data['amount']} BDT\n"                 f"🔑 **Withdrawal Code:** {user_data['withdrawal_code']}\n\n"                 f"✅ Click **SUBMIT** to proceed.") keyboard = InlineKeyboardMarkup().add(     InlineKeyboardButton("✅ SUBMIT", callback_data="submit_withdraw"),     InlineKeyboardButton("❌ CANCEL", callback_data="cancel") ) await message.answer(confirm_text, reply_markup=keyboard) await WithdrawForm.confirmation.set() ` 
# ✅ Handle Confirmation
 
@dp.callback_query_handler(lambda query: query.data == "submit_withdraw", state=WithdrawForm.confirmation) async def confirm_withdraw_submission(callback_query: types.CallbackQuery, state: FSMContext): user_data = await state.get_data() await bot.send_message(user_data["user_id"], "✅ **Your withdrawal request is under processing!**")
 `admin_text = (f"📌 **New Withdrawal Request**\n\n"               f"👤 **User ID:** {user_data['user_id']}\n"               f"👤 **Player ID:** {user_data['player_id']}\n"               f"📛 **Name:** {user_data['name']}\n"               f"🏦 **Payment Method:** {user_data['payment_method'].upper()}\n"               f"📞 **Wallet Number:** {user_data['wallet_number']}\n"               f"💰 **Amount:** {user_data['amount']} BDT\n"               f"🔑 **Withdrawal Code:** {user_data['withdrawal_code']}\n\n"               f"✅ **Approve or Reject Below.**") admin_keyboard = InlineKeyboardMarkup().add(     InlineKeyboardButton("✅ APPROVE", callback_data=f"approve_withdraw_{user_data['user_id']}"),     InlineKeyboardButton("❌ REJECT", callback_data=f"reject_withdraw_{user_data['user_id']}") ) await bot.send_message(config.ADMIN_GROUP_ID, admin_text, reply_markup=admin_keyboard) await state.finish() ` 
# ✅ Handle Admin Approval/Rejection
 
@dp.callback_query_handler(lambda query: query.data.startswith("approve_withdraw") or query.data.startswith("reject_withdraw")) async def finalize_withdrawal_decision(callback_query: types.CallbackQuery): data_parts = callback_query.data.split("_") decision = data_parts[0]  # "approve" or "reject" user_id = int(data_parts[2]) action_text = "✅ **Withdrawal Approved**" if decision == "approve" else "❌ **Withdrawal Rejected**" await bot.send_message(user_id, action_text) if decision == "approve": await bot.send_message(config.HISTORY_GROUP_ID, callback_query.message.text) await callback_query.message.edit_text(action_text)

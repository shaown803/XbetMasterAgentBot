...
def deposit_method_selected(client, callback_query):
    method = callback_query.data.split("_")[1]  # Extract payment method
    print(f"✅ DEBUG: Payment method selected: {method}")  # Debug print

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Done", callback_data=f"deposit_done_{method}")]
    ])
    
    callback_query.message.edit_text(
        f"📌 **Selected Payment Method:** {method.capitalize()}\n"
...
def deposit_submission_form(client, callback_query):
    print(f"✅ DEBUG: deposit_submission_form triggered with data: {callback_query.data}")

    method = callback_query.data.split("_")[2].capitalize()  # Fix indexing issue
    print(f"✅ DEBUG: Extracted method: {method}")  # Debug print
    ...
    callback_query.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Submit", callback_data=f"confirm_deposit_{method.lower()}")]])
    )

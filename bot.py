...
def deposit_submission_form(client, callback_query):
    ...
    client.send_message(callback_query.message.chat.id, new_text, reply_markup=keyboard)
...
# ✅ Step 5: Send Deposit Request to Admin Group
...
# ✅ Step 6: Admin Confirms Before Approving
...
def confirm_admin_approve(client, callback_query):
    ...
    callback_query.message.edit_text("⚠️ **Are you sure you want to approve this deposit request?**", reply_markup=keyboard)

# ✅ Step 7: Admin Confirms Before Rejecting
...
def confirm_admin_reject(client, callback_query):
    ...
    callback_query.message.edit_text("⚠️ **Are you sure you want to reject this deposit request?**", reply_markup=keyboard)

# ✅ Step 8: Admin Approves Deposit
@app.on_callback_query(filters.regex("^approve_deposit_(.+)"))
def approve_deposit(client, callback_query):
    user_id = int(callback_query.data.split("_")[2])

    client.send_message(DEPOSIT_GROUP_ID, f"✅ **Deposit Approved!**\n\nUser: [{user_id}](tg://user?id={user_id})\n💰 Your deposit has been successfully processed!")
    client.send_message(user_id, "✅ **Your deposit request has been approved!**\nThe amount should now be added to your balance.")
    callback_query.message.edit_text("✅ Deposit Approved!")

# ✅ Step 9: Admin Rejects Deposit
@app.on_callback_query(filters.regex("^reject_deposit_(.+)"))
def reject_deposit(client, callback_query):
    user_id = int(callback_query.data.split("_")[2])

    client.send_message(DEPOSIT_GROUP_ID, f"❌ **Deposit Rejected!**\n\nUser: [{user_id}](tg://user?id={user_id})\n🚫 Your deposit request has been denied.")
    client.send_message(user_id, "❌ **Your deposit request has been rejected!**\nPlease contact support for more details.")
    callback_query.message.edit_text("❌ Deposit Rejected!")

# ✅ Step 10: Cancel Actions

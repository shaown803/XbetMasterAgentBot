...
class WithdrawalForm(StatesGroup):
    ...
    wallet_number = State()
    amount = State()
    withdrawal_code = State()
    confirmation = State()

# ✅ Handle "Withdraw" Button Click
@dp.message_handler(lambda message: message.text == "💳 Withdraw")
async def show_withdrawal_address(message: types.Message):
    withdrawal_address = (
        "📍 **Withdrawal Address**\n"
        "City: Chittagong\n"
        "Street: Dorbesh Baba\n\n"
    ...
    keyboard = InlineKeyboardMarkup().add(InlineKeyboardButton("➡ Next", callback_data="next_withdrawal"))
    await message.answer(withdrawal_address, reply_markup=keyboard)

# ✅ Handle "Next" Button Click after Withdrawal Address
@dp.callback_query_handler(lambda query: query.data == "next_withdrawal")
async def select_payment_method(callback_query: types.CallbackQuery):
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("📌 UPAY", callback_data="withdraw_upay"),
        InlineKeyboardButton("❌ Cancel", callback_data="cancel")
    )
    await callback_query.message.edit_text("Select a payment method:", reply_markup=keyboard)

# ✅ Handle Payment Method Selection for Withdrawal
@dp.callback_query_handler(lambda query: query.data.startswith("withdraw_"))
async def start_withdrawal_form(callback_query: types.CallbackQuery, state: FSMContext):
    payment_method = callback_query.data.replace("withdraw_", "").upper()
    await state.update_data(payment_method=payment_method, user_id=callback_query.from_user.id)

    await bot.send_message(callback_query.from_user.id, "1️⃣ Send your **Player ID**:")
    await WithdrawalForm.player_id.set()

# ✅ Step 1: Collect Player ID for Withdrawal
@dp.message_handler(state=WithdrawalForm.player_id)
async def process_player_id(message: types.Message, state: FSMContext):
    await state.update_data(player_id=message.text)
    await message.answer("2️⃣ Enter your **Name**:")
    await WithdrawalForm.name.set()

# ✅ Step 2: Collect Name for Withdrawal
@dp.message_handler(state=WithdrawalForm.name)
async def process_name(message: types.Message, state: FSMContext):
    await state.update_data(name=message.text)

    # Fetch payment method from state
    user_data = await state.get_data()
    payment_method = user_data["payment_method"]

    await message.answer(f"3️⃣ Payment Method: **{payment_method}** (Auto-filled)\n\n4️⃣ Enter your **Wallet Number**:")
    await WithdrawalForm.wallet_number.set()

# ✅ Step 3: Collect Wallet Number for Withdrawal
@dp.message_handler(state=WithdrawalForm.wallet_number)
async def process_wallet_number(message: types.Message, state: FSMContext):
    await state.update_data(wallet_number=message.text)
    await message.answer("5️⃣ Enter the **Amount** you want to withdraw:")
    await WithdrawalForm.amount.set()

# ✅ Step 4: Collect Amount for Withdrawal
@dp.message_handler(state=WithdrawalForm.amount)
async def process_amount(message: types.Message, state: FSMContext):
    await state.update_data(amount=message.text)
    await message.answer("6️⃣ Enter the **Withdrawal Code**:")
    await WithdrawalForm.withdrawal_code.set()

# ✅ Step 5: Collect Withdrawal Code for Withdrawal
@dp.message_handler(state=WithdrawalForm.withdrawal_code)
async def process_withdrawal_code(message: types.Message, state: FSMContext):
    await state.update_data(withdrawal_code=message.text)
    user_data = await state.get_data()
    ...
    confirm_text = (f"📌 **Withdrawal Submission**\n\n"
                    ...
                    f"🏦 **Payment Method:** {user_data['payment_method']}\n"
                    f"📞 **Wallet Number:** {user_data['wallet_number']}\n"
                    f"💰 **Amount:** {user_data['amount']} BDT\n"
...
# ✅ Handle Withdrawal Submission Confirmation
@dp.callback_query_handler(lambda query: query.data == "submit_withdrawal", state=WithdrawalForm.confirmation)
async def confirm_withdrawal_submission(callback_query: types.CallbackQuery, state: FSMContext):
    user_data = await state.get_data()

    # Notify user
    await bot.send_message(user_data["user_id"], "✅ **Your withdrawal request is under processing!**")

    # Send to admin group for approval/rejection
...
# ✅ Handle Admin Approval/Rejection for Withdrawal
@dp.callback_query_handler(lambda query: query.data.startswith("approve_withdrawal") or query.data.startswith("reject_withdrawal"))
async def finalize_admin_decision_withdrawal(callback_query: types.CallbackQuery):
    data_parts = callback_query.data.split("_")
    decision = data_parts[0]  # "approve" or "reject"
    user_id = int(data_parts[2])

    action_text = "✅ **Withdrawal Approved**" if decision == "approve" else "❌ **Withdrawal Rejected**"

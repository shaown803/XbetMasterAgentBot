...
# # ✅ Handle Admin Approval/Rejection Click
...
async def finalize_admin_decision(callback_query: types.CallbackQuery):
    ...
    # Get user ID from the original message text
    message_lines = callback_query.message.text.split("\n")
    player_id_line = next((line for line in message_lines if "👤 **Player ID:**" in line), None)
    
    if player_id_line:
        player_id = player_id_line.replace("👤 **Player ID:**", "").strip()
        user_id = int(player_id)  # Ensure it's an integer

        # Notify user
        await bot.send_message(user_id, action)

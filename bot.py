...
class WithdrawalForm(StatesGroup): player_id = State() name = State() payment_method = State() wallet_number = State() amount = State() withdrawal_code = State() confirmation = State()
 
# ✅ Withdrawal Menu
 
async def show_withdrawal_address(message: types.Message): text = "🏦 **Withdrawal Address**\n\nCity: Chittagong\nStreet: Dorbesh Baba" keyboard = InlineKeyboardMarkup().add(InlineKeyboardButton("➡ Next", callback_data="withdraw_next")) await message.answer(text, reply_markup=keyboard)
 
# ✅ Handle Withdraw Button Click
 
@dp.message_handler(lambda message: message.text == "💳 Withdraw") async def initiate_withdrawal(message: types.Message): await show_withdrawal_address(message)
 
# ✅ Handle "Next" Button Click to Select Payment Method
 
@dp.callback_query_handler(lambda query: query.data == "withdraw_next") async def ask_payment_method(callback_query: types.CallbackQuery): keyboard = InlineKeyboardMarkup(row_width=2) keyboard.add( InlineKeyboardButton("📌 BKASH", callback_data="withdraw_bkash"), InlineKeyboardButton("📌 NAGAD", callback_data="withdraw_nagad"), InlineKeyboardButton("📌 ROCKET", callback_data="withdraw_rocket"), InlineKeyboardButton("📌 UPAY", callback_data="withdraw_upay") ) await callback_query.message.edit_text("Select a payment method:", reply_markup=keyboard)
 
# ✅ Handle Payment Method Selection
 
@dp.callback_query_handler(lambda query: query.data.startswith("withdraw_")) async def start_withdrawal_form(callback_query: types.CallbackQuery, state: FSMContext): payment_method = callback_query.data.split("_")[1] await state.update_data(payment_method=payment_method, user_id=callback_query.from_user.id)
 `await bot.send_message(callback_query.from_user.id, "1️⃣ Send your **Player ID**:") await WithdrawalForm.player_id.set() ` 
# ✅ Step 1: Collect Player ID
 
@dp.message_handler(state=WithdrawalForm.player_id) async def process_player_id(message: types.Message, state: FSMContext): await state.update_data(player_id=message.text) await message.answer("2️⃣ Enter your **Name**:") await WithdrawalForm.name.set()
 
# ✅ Step 2: Collect Name
 
@dp.message_handler(state=WithdrawalForm.name) async def process_name(message: types.Message, state: FSMContext): await state.update_data(name=message.text) user_data = await state.get_data() await message.answer(f"3️⃣ Payment Method: **{user_data['payment_method'].upper()}** (Auto-filled)\n\n4️⃣ Enter your **Wallet Number**:") await WithdrawalForm.wallet_number.set()
 
# ✅ Step 3: Collect Wallet Number
 
@dp.message_handler(state=WithdrawalForm.wallet_number) async def process_wallet_number(message: types.Message, state: FSMContext): await state.update_data(wallet_number=message.text) await message.answer("5️⃣ Enter the **Amount**:") await WithdrawalForm.amount.set()
 
# ✅ Step 4: Collect Amount
 
@dp.message_handler(state=WithdrawalForm.amount) async def process_amount(message: types.Message, state: FSMContext): await state.update_data(amount=message.text) await message.answer("6️⃣ Enter your **Withdrawal Code**:") await WithdrawalForm.withdrawal_code.set()
 
# ✅ Step 5: Collect Withdrawal Code
 
@dp.message_handler(state=WithdrawalForm.withdrawal_code) async def process_withdrawal_code(message: types.Message, state: FSMContext): await state.update_data(withdrawal_code=message.text) user_data = await state.get_data()
 `confirm_text = (f"📌 **Withdrawal Submission**\n\n"                 f"👤 **Player ID:** {user_data['player_id']}\n"                 f"👤 **Name:** {user_data['name']}\n"                 f"🏦 **Payment Method:** {user_data['payment_method'].upper()}\n"                 f"📞 **Wallet Number:** {user_data['wallet_number']}\n"                 f"💰 **Amount:** {user_data['amount']} BDT\n"                 f"🔑 **Withdrawal Code:** {user_data['withdrawal_code']}\n\n"                 f"✅ **Click SUBMIT to proceed.**")  keyboard = InlineKeyboardMarkup().add(     InlineKeyboardButton("✅ SUBMIT", callback_data="submit_withdraw"),     InlineKeyboardButton("❌ CANCEL", callback_data="cancel") ) await message.answer(confirm_text, reply_markup=keyboard) await WithdrawalForm.confirmation.set() ` 
# ✅ Handle Confirmation
 
@dp.callback_query_handler(lambda query: query.data == "submit_withdraw", state=WithdrawalForm.confirmation) async def confirm_withdrawal_submission(callback_query: types.CallbackQuery, state: FSMContext): user_data = await state.get_data() await bot.send_message(user_data["user_id"], "✅ **Your withdrawal request is under processing!**")
